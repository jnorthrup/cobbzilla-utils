...
public class TarballTest {
    ...
    @Test public void testUnrollAndReroll () throws Exception {
        ...
        final File tarball = StreamUtil.INSTANCE.stream2temp(StreamUtil.loadResourceAsStream(StringUtil.INSTANCE.packagePath(getClass()) + "/test.tar.gz"));

        // unroll the tarball
        Tarball.INSTANCE.unroll(tarball, tempDir);

        // list files in tempDir, we expect file1 and subdir
        validateUnrolledTarball(tempDir);

        final File newTar = File.createTempFile("temp", ".tar", INSTANCE.getDefaultTempDir());
    ...
    protected void validateUnrolledTarball(File unrolledDir) throws IOException {
        ...
        shasum = StringUtil.tohex(ShaUtil.INSTANCE.sha256(FileUtil.INSTANCE.toString(files[fileIndex])));
        ...
        shasum = StringUtil.tohex(ShaUtil.INSTANCE.sha256(FileUtil.INSTANCE.toString(subdirFiles[0])));
